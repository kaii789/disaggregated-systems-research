...
CompressionModelFVECounter::CompressionModelFVECounter(String name, UInt32 page_size, UInt32 cache_line_size)
    : m_name(name)
    , m_page_size(page_size)
    , m_cache_line_size(cache_line_size)
{
    // Set compression/decompression cycle latencies if configured
    if (Sim()->getCfg()->getInt("perf_model/dram/compression_model/fve_counter/compression_latency") != -1)
        m_compression_latency = Sim()->getCfg()->getInt("perf_model/dram/compression_model/fve_counter/compression_latency");
    if (Sim()->getCfg()->getInt("perf_model/dram/compression_model/fve_counter/decompression_latency") != -1)
        m_decompression_latency = Sim()->getCfg()->getInt("perf_model/dram/compression_model/fve_counter/decompression_latency");
   ...
   _wordsize = 32;
   CAM_C = new CAM_counter(128);
   CAM_D = new CAM_counter(128);

}

SubsecondTime
CompressionModelFVECounter::compress(IntPtr addr, size_t data_size, core_id_t core_id, UInt32 *compressed_page_size, UInt32 *compressed_cache_lines)
{
    // Get Data
...
CompressionModelFVECounter::~CompressionModelFVECounter()
{
	delete CAM_C;
	delete CAM_D;
}

UInt32 CompressionModelFVECounter::compressCacheLine(void* _inbuf, void* _outbuf)
{
    bitstream stream;
...
SubsecondTime
CompressionModelFVECounter::decompress(IntPtr addr, UInt32 compressed_cache_lines, core_id_t core_id)
{
    Core *core = Sim()->getCoreManager()->getCoreFromID(core_id);
...
Byte* CompressionModelFVECounter::MakeCompBuf()
{
   Byte* buf = new Byte[m_cache_line_size+2]();
   return buf; 
}

void CompressionModelFVECounter::InitBitstream( bitstream *stream,
...
unsigned long int CompressionModelFVECounter::ReadBit(bitstream *stream )
{
    unsigned int idx;
...
void CompressionModelFVECounter::WriteBit( bitstream *stream, unsigned  long int x  ) /*Αλλάζω, για φορά LSB->MSB από rice όπου ήταν MSB->LSB*/ 
{
...
void CompressionModelFVECounter::EncodeWord(bitstream *stream, unsigned long int x, unsigned int indx)
{
...
unsigned long int CompressionModelFVECounter::DecodeWord(bitstream *stream, unsigned int indx)
{
    ...
    /*Read from header if this is compressed or not*/
     ...
     if(compressed!=0)
     {
        ...
        std::pair <bool,unsigned char> result(1,cam_indx);
        CAM_D->Update1(&result,x);
...
unsigned long int CompressionModelFVECounter::ReadWord(void *ptr, unsigned int idx)	//idx says which word of input ptr to read ( i from main code/decode loop)
{
    unsigned long int   x;
...
void CompressionModelFVECounter::WriteWord( void *ptr, unsigned int idx, unsigned long int x )
{
...
UInt32 CompressionModelFVECounter::decompressCacheLine(void *in, void *out)
{
    bitstream stream;
