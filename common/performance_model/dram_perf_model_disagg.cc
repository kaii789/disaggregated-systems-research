...
//   , m_memory_manager(memory_manager)
   ...
   , m_r_part_bandwidth       (m_dram_speed * m_data_bus_width /(2* 1000 * Sim()->getCfg()->getFloat("perf_model/dram/remote_mem_bw_scalefactor"))) // Remote memory
   , m_r_part2_bandwidth       (m_dram_speed * m_data_bus_width /(2* 1000 * Sim()->getCfg()->getFloat("perf_model/dram/remote_mem_bw_scalefactor"))) // Remote memory
   ...
   , m_r_added_latency       (SubsecondTime::NS() * static_cast<uint64_t> (Sim()->getCfg()->getFloat("perf_model/dram/remote_mem_add_lat"))) // Enable remote memory
   ...
   , m_enable_remote_mem       (Sim()->getCfg()->getBool("perf_model/dram/enable_remote_mem")) // Enable remote memory
   , m_r_simulate_tlb_overhead       (Sim()->getCfg()->getBool("perf_model/dram/simulate_tlb_overhead")) // Enable remote memory
   , m_r_simulate_datamov_overhead       (Sim()->getCfg()->getBool("perf_model/dram/simulate_datamov_overhead")) // Enable remote memory
   , m_r_mode       (Sim()->getCfg()->getInt("perf_model/dram/remote_memory_mode"))// Move data if greater than
   ...
   , m_r_exclusive_cache       (Sim()->getCfg()->getBool("perf_model/dram/remote_exclusive_cache"))// Add 2600usec delay
   , m_remote_init       (Sim()->getCfg()->getBool("perf_model/dram/remote_init"))// Add 2600usec delay
   , m_r_enable_nl_prefetcher       (Sim()->getCfg()->getBool("perf_model/dram/enable_remote_prefetcher"))// Add 2600usec delay
   , m_r_disturbance_factor      (Sim()->getCfg()->getInt("perf_model/dram/remote_disturbance_factor"))// Add 2600usec delay
   , m_r_dontevictdirty      (Sim()->getCfg()->getBool("perf_model/dram/remote_dontevictdirty"))// Add 2600usec delay
   , m_r_enable_selective_moves      (Sim()->getCfg()->getBool("perf_model/dram/remote_enable_selective_moves"))// Add 2600usec delay
   , m_r_partition_queues     (Sim()->getCfg()->getInt("perf_model/dram/remote_partitioned_queues"))// Add 2600usec delay
   , m_r_cacheline_gran      (Sim()->getCfg()->getBool("perf_model/dram/remote_use_cacheline_granularity"))// Add 2600usec delay
   , m_r_reserved_bufferspace      (Sim()->getCfg()->getInt("perf_model/dram/remote_reserved_buffer_space"))// Add 2600usec delay
   , m_r_limit_redundant_moves      (Sim()->getCfg()->getInt("perf_model/dram/remote_limit_redundant_moves"))// Add 2600usec delay
   , m_r_throttle_redundant_moves      (Sim()->getCfg()->getBool("perf_model/dram/remote_throttle_redundant_moves"))// Add 2600usec delay
...
//		move_page = false;
  // } 
   ...
   // LOG_PRINT("move_page is %d", move_page);
   ...
   */ 
   ...
   // LOG_PRINT("Final remote access latency: %ld ns", (t_now - pkt_time).getNS());
...
DramPerfModelDisagg::getAccessLatency(SubsecondTime pkt_time, UInt64 pkt_size, core_id_t requester, IntPtr address, DramCntlrInterface::access_t access_type, ShmemPerf *perf)
{
   ...
   //std::cout << "Inflight Pages: " << std::endl;  
   /*
   for( auto const& i : m_inflight_pages) { // cgiannoula
   //std::cout << std::hex << i.first << ": " << i.second << ": " << (i.second > pkt_time? (i.second - pkt_time): SubsecondTime::Zero() ) << std::endl; 
   //if(i.second <= std::max(Sim()->getClockSkewMinimizationServer()->getGlobalTime(),pkt_time)) {
   if(i.second <= SubsecondTime::max(Sim()->getClockSkewMinimizationServer()->getGlobalTime(),pkt_time)) {
         m_inflight_pages.erase(i.first);
      m_inflight_redundant.erase(i.first);
   } 
   }
   //std::cout << "Inflight Evicted Pages: " << std::endl;  
   for( auto const& i : m_inflightevicted_pages) {
   //std::cout << std::hex << i.first << ": " << i.second << std::endl; 
   //if(i.second <= std::max(Sim()->getClockSkewMinimizationServer()->getGlobalTime(),pkt_time))
   if(i.second <= SubsecondTime::max(Sim()->getClockSkewMinimizationServer()->getGlobalTime(),pkt_time))
         m_inflightevicted_pages.erase(i.first);
   }
   */
   // cgiannoula
   ...
   /*std::cout << "Local Pages: " << std::endl;  
   for( auto const& i : m_local_pages) {
	std::cout << std::hex << i << std::endl; 
   }
   std::cout << "Remote Pages: " << std::endl;  
   for( auto const& i : m_remote_pages) {
	std::cout << std::hex << i << std::endl; 
   }*/
   ...
   //Nandita: Should we enable a remote access?
   ...
   if((m_inflight_pages.find(phys_page) == m_inflight_pages.end()) || m_r_enable_selective_moves)
    //if (m_r_enable_selective_moves) {
      ...
      if(access_latency > (t_now - pkt_time)) {
         m_inflight_hits++; 
         //std::cout << "Result: Inflight Hit" << std::endl;  
...
bool DramPerfModelDisagg::isRemoteAccess(IntPtr address, core_id_t requester, DramCntlrInterface::access_t access_type) {
   ...
   if(m_r_cacheline_gran) //When we perform moves at cacheline granularity (should be disabled by default)
      num_local_pages = m_localdram_size/64;  
   //std::cout << "Result: Remote" << std::endl;  
   ...
   if(m_r_mode == 0 || m_r_mode == 4) { //Static partitioning: no data movement and m_r_partitioning_ratio decides how many go where
      ...
      if (std::find(m_local_pages.begin(), m_local_pages.end(), phys_page) != m_local_pages.end()) { // is it in local dram?
         ...
         if(m_remote_init) { //Assuming all pages start off in remote memory
            ...
            // if (std::find(m_remote_pages.begin(), m_remote_pages.end(), phys_page) != m_remote_pages.end()) 
            //    printf("Remote page found: %lx\n", phys_page); 
            return true;
         }
         else {
...
SubsecondTime DramPerfModelDisagg::possiblyEvict(UInt64 phys_page, SubsecondTime t_now, core_id_t requester) {
   ...
   if(m_local_pages.size() > num_local_pages) {
	...
	if(!found) {
		evicted_page = m_local_pages.front(); 
   ...
   if (std::find(m_dirty_pages.begin(), m_dirty_pages.end(), evicted_page) != m_dirty_pages.end()) {
	++m_data_moves;
        ++m_writeback_pages;
	SubsecondTime page_datamovement_queue_delay;
        if(m_r_simulate_datamov_overhead) { 
		if(m_r_partition_queues)
			page_datamovement_queue_delay = m_data_movement->computeQueueDelay(t_now, m_r_part_bandwidth.getRoundedLatency(8*4096), requester);
...
void DramPerfModelDisagg::possiblyPrefetch(UInt64 phys_page, SubsecondTime t_now, core_id_t requester) {
