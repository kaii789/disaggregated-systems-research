...
DramPerfModelDisaggPageMoveFree::DramPerfModelDisaggPageMoveFree(core_id_t core_id, UInt32 cache_block_size, AddressHomeLookup* address_home_lookup)
    ...
    , m_local_total_remote_access_latency(SubsecondTime::Zero())
{
    ...
    // For debugging
    std::cout << "dram_perf_model_disagg_page_move_free.cc" << std::endl;
    if (m_r_partition_queues)
...
template<typename T>
void DramPerfModelDisaggPageMoveFree::sortAndPrintVectorPercentiles(std::vector<T>& vec, std::ostringstream& percentages_buffer, std::ostringstream& counts_buffer, UInt32 num_bins)
{
...
void
DramPerfModelDisaggPageMoveFree::finalizeStats() 
{
...
UInt64
DramPerfModelDisaggPageMoveFree::parseAddressBits(UInt64 address, UInt32 &data, UInt32 offset, UInt32 size, UInt64 base_address = 0)
{
...
void
DramPerfModelDisaggPageMoveFree::parseDeviceAddress(IntPtr address, UInt32 &channel, UInt32 &rank, UInt32 &bank_group, UInt32 &bank, UInt32 &column, UInt64 &dram_page)
{
...
SubsecondTime
DramPerfModelDisaggPageMoveFree::getDramWriteCost(SubsecondTime start_time, UInt64 size, core_id_t requester, IntPtr address, ShmemPerf *perf, bool is_exclude_cacheline, bool is_page)
{
...
// DRAM hardware access cost
SubsecondTime
DramPerfModelDisaggPageMoveFree::getDramAccessCost(SubsecondTime start_time, UInt64 size, core_id_t requester, IntPtr address, ShmemPerf *perf, bool is_remote, bool is_exclude_cacheline, bool is_page)
{
    SubsecondTime t_now = start_time;
    SubsecondTime dram_access_cost = m_dram_hw_fixed_latency;
...
SubsecondTime
DramPerfModelDisaggPageMoveFree::getAccessLatencyRemote(SubsecondTime pkt_time, UInt64 pkt_size, core_id_t requester, IntPtr address, DramCntlrInterface::access_t access_type, ShmemPerf *perf)
{
    ...
    if (move_page) {
        ...
        SubsecondTime page_arrival_time = t_remote_queue_request + cacheline_network_processing_time + cacheline_queue_delay;  // m_r_simulate_datamov_overhead == false: assume the whole page arrives in local dram when the cacheline arrives
        ...
        /*} else*/ if (m_r_partition_queues == 0 && move_page && m_data_movement->getPageQueueUtilizationPercentage(t_now) > m_r_page_queue_utilization_threshold) {
            ...
            if (m_r_simulate_datamov_overhead)
                ...
                m_inflight_pages[phys_page] = page_arrival_time;  // assume the whole page arrives in local dram when the cacheline arrives
                // m_inflight_pages[phys_page] = SubsecondTime::max(global_time, page_arrival_time);  // assume the whole page arrives in local dram when the cacheline arrives
                // m_inflight_pages[phys_page] = t_remote_queue_request;  // instant arrival, potentially even before the cacheline arrives
...
void
DramPerfModelDisaggPageMoveFree::updateLocalIPCStat()
{
...
void
DramPerfModelDisaggPageMoveFree::update_local_remote_latency_stat(SubsecondTime access_latency)
{
...
void
DramPerfModelDisaggPageMoveFree::updateBandwidth()
{
    ...
    if (m_use_dynamic_bandwidth && m_update_bandwidth_count % 20 == 0) {
        ...
        // Same formulas here as in DramPerfModelDisaggPageMoveFree constructor
        m_r_bus_bandwidth.changeBandwidth(m_dram_speed * m_data_bus_width / (1000 * m_r_bw_scalefactor)); // Remote memory
        m_r_part_bandwidth.changeBandwidth(m_dram_speed * m_data_bus_width / (1000 * m_r_bw_scalefactor / (1 - m_r_cacheline_queue_fraction))); // Remote memory - Partitioned Queues => Page Queue
        m_r_part2_bandwidth.changeBandwidth(m_dram_speed * m_data_bus_width / (1000 * m_r_bw_scalefactor / m_r_cacheline_queue_fraction)); // Remote memory - Partitioned Queues => Cacheline Queue
        // Currently only windowed_mg1_remote_ind_queues QueueModel updates stats tracking based on updateBandwidth()
...
void
DramPerfModelDisaggPageMoveFree::updateLatency()
{
...
void
DramPerfModelDisaggPageMoveFree::update_bw_utilization_count(SubsecondTime pkt_time)
{
...
SubsecondTime
DramPerfModelDisaggPageMoveFree::getAccessLatency(SubsecondTime pkt_time, UInt64 pkt_size, core_id_t requester, IntPtr address, DramCntlrInterface::access_t access_type, ShmemPerf *perf)
{
...
bool
DramPerfModelDisaggPageMoveFree::isRemoteAccess(IntPtr address, core_id_t requester, DramCntlrInterface::access_t access_type) 
{
...
SubsecondTime 
DramPerfModelDisaggPageMoveFree::possiblyEvict(UInt64 phys_page, SubsecondTime t_now, core_id_t requester) 
{
    ...
    if (m_local_pages.size() > num_local_pages) {
        ...
        if (m_dirty_pages.count(evicted_page)) {
            ...
            if (m_r_simulate_datamov_overhead)
                ...
                m_inflightevicted_pages[evicted_page] = t_remote_queue_request;
...
void 
DramPerfModelDisaggPageMoveFree::possiblyPrefetch(UInt64 phys_page, SubsecondTime t_now, core_id_t requester) 
{
    // Important note: phys_page is the current physical page being accessed before the call to this function,
