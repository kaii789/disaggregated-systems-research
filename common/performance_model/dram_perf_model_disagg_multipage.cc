...
DramPerfModelDisaggMultipage::DramPerfModelDisaggMultipage(core_id_t core_id, UInt32 cache_block_size, AddressHomeLookup* address_home_lookup)
    ...
    , m_redundant_moves_type1_cache_slower_than_page (0)
    ...
    , m_total_remote_access_latency(SubsecondTime::Zero())
{
    ...
    // Multipage
    m_page_buffer_capacity = Sim()->getCfg()->getInt("perf_model/dram/multipage/page_buffer_capacity");
    ...
    if (m_r_partition_queues) {
        ...
        registerStatsMetric("dram", core_id, "redundant-moves-type1-cache-slower-than-page", &m_redundant_moves_type1_cache_slower_than_page);
...
DramPerfModelDisaggMultipage::~DramPerfModelDisaggMultipage()
{
...
UInt64
DramPerfModelDisaggMultipage::parseAddressBits(UInt64 address, UInt32 &data, UInt32 offset, UInt32 size, UInt64 base_address = 0)
{
...
void
DramPerfModelDisaggMultipage::parseDeviceAddress(IntPtr address, UInt32 &channel, UInt32 &rank, UInt32 &bank_group, UInt32 &bank, UInt32 &column, UInt64 &dram_page)
{
...
SubsecondTime
DramPerfModelDisaggMultipage::getAccessLatencyRemote(SubsecondTime pkt_time, UInt64 pkt_size, core_id_t requester, IntPtr address, DramCntlrInterface::access_t access_type, ShmemPerf *perf)
{
    ...
    if (m_r_partition_queues == 1) {
        datamovement_queue_delay = m_data_movement_2->computeQueueDelayTrackBytes(t_now, m_r_part2_bandwidth.getRoundedLatency(8*size), size, QueueModel::CACHELINE, requester);
        m_redundant_moves++;
        ++m_redundant_moves_type1;
    } else {
        datamovement_queue_delay = m_data_movement->computeQueueDelayTrackBytes(t_now, m_r_bus_bandwidth.getRoundedLatency(8*size), size, QueueModel::CACHELINE, requester);
    }
    ...
    if (move_page) {
        ...
        if (page_buffer.size() == m_page_buffer_capacity) {
            ...
            // Compute how long it would take to move combined pages
            ...
            if(m_r_simulate_datamov_overhead && !m_r_cacheline_gran) {
                ...
                if(m_r_partition_queues == 1) {
                    page_datamovement_queue_delay = m_data_movement->computeQueueDelayTrackBytes(t_now, m_r_part_bandwidth.getRoundedLatency(8*page_size), page_size, QueueModel::PAGE, requester);

                    // Approximation of time savings:
                    SubsecondTime alternative_page_delay = m_data_movement->computeQueueDelayNoEffect(t_now, m_r_bus_bandwidth.getRoundedLatency(8*page_size), QueueModel::PAGE, requester);
                    ...
                    page_datamovement_queue_delay = m_data_movement->computeQueueDelayTrackBytes(t_now, m_r_bus_bandwidth.getRoundedLatency(8*page_size), page_size, QueueModel::PAGE, requester);
                ...
                if (m_use_compression)
                {
                    SubsecondTime decompression_latency = m_compression_model->decompress_multipage(page_buffer, m_page_buffer_capacity, m_core_id, &address_to_num_cache_lines); // TODO: decompress_multipage
            ...
            for (UInt32 i = 0; i < m_page_buffer_capacity; i++) {
                UInt64 phys_page = page_buffer.back();
                ...
                t_now += possiblyEvict(phys_page, pkt_time, requester); //Check if there's place in local DRAM and if not evict an older page to make space
            }
        }
    }
...
SubsecondTime
DramPerfModelDisaggMultipage::getAccessLatency(SubsecondTime pkt_time, UInt64 pkt_size, core_id_t requester, IntPtr address, DramCntlrInterface::access_t access_type, ShmemPerf *perf)
{
    UInt64 phys_page = address & ~((UInt64(1) << floorLog2(m_page_size)) - 1);
    if(m_r_cacheline_gran) 
        phys_page =  address & ~((UInt64(1) << floorLog2(m_cache_line_size)) - 1); // Was << 6
            ...
            if(m_r_partition_queues == 1) {
                if (m_inflight_redundant[phys_page] < m_r_limit_redundant_moves) {
                    ...
                    if(m_r_throttle_redundant_moves) {
                        ...
                        SubsecondTime datamov_queue_delay = m_data_movement_2->computeQueueDelayNoEffect(t_now, m_r_part2_bandwidth.getRoundedLatency(8*pkt_size), QueueModel::CACHELINE, requester);
                        if ((datamov_queue_delay + t_now - pkt_time) < access_latency) {
                            datamov_queue_delay = m_data_movement_2->computeQueueDelayTrackBytes(t_now, m_r_part2_bandwidth.getRoundedLatency(8*pkt_size), pkt_size, QueueModel::CACHELINE, requester);
                            ++m_redundant_moves;
                        ...
                        SubsecondTime datamov_queue_delay = m_data_movement_2->computeQueueDelayTrackBytes(t_now, m_r_part2_bandwidth.getRoundedLatency(8*pkt_size), pkt_size, QueueModel::CACHELINE, requester);
                        ++m_redundant_moves;
                        ++m_redundant_moves_type2;
                        m_inflight_redundant[phys_page] = m_inflight_redundant[phys_page] + 1; 
                        if ((datamov_queue_delay + t_now - pkt_time) < access_latency) {
...
bool 
DramPerfModelDisaggMultipage::isRemoteAccess(IntPtr address, core_id_t requester, DramCntlrInterface::access_t access_type) 
{
...
SubsecondTime 
DramPerfModelDisaggMultipage::possiblyEvict(UInt64 phys_page, SubsecondTime t_now, core_id_t requester) 
{
    ...
    if(m_local_pages.size() > num_local_pages) {
        ...
        if (std::find(m_dirty_pages.begin(), m_dirty_pages.end(), evicted_page) != m_dirty_pages.end()) {
            ...
            if(m_r_simulate_datamov_overhead) { 
                if(m_r_partition_queues == 1) {
                    page_datamovement_queue_delay = m_data_movement->computeQueueDelayTrackBytes(t_now, m_r_part_bandwidth.getRoundedLatency(8*size), size, queue_request_type, requester);
                } /* else if(m_r_cacheline_gran) {
                    page_datamovement_queue_delay = m_data_movement->computeQueueDelayTrackBytes(t_now, m_r_bus_bandwidth.getRoundedLatency(8*size), size, queue_request_type, requester);
                } */ else {
                    page_datamovement_queue_delay = m_data_movement->computeQueueDelayTrackBytes(t_now, m_r_bus_bandwidth.getRoundedLatency(8*size), size, queue_request_type, requester);
                }
            }
...
void 
DramPerfModelDisaggMultipage::possiblyPrefetch(UInt64 phys_page, SubsecondTime t_now, core_id_t requester) 
{
    ...
    if (std::find(m_local_pages.begin(), m_local_pages.end(), pref_page) == m_local_pages.end() && std::find(m_remote_pages.begin(), m_remote_pages.end(), pref_page) != m_remote_pages.end()) {
        ...
        SubsecondTime page_datamovement_queue_delay = SubsecondTime::Zero();
        if(m_r_simulate_datamov_overhead) { 
            page_datamovement_queue_delay = m_data_movement->computeQueueDelayTrackBytes(t_now, m_r_bus_bandwidth.getRoundedLatency(8*m_page_size), m_page_size, queue_request_type, requester);
        } 
