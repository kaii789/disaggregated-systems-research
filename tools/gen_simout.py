...
def generate_simout(jobid = None, resultsdir = None, partial = None, output = sys.stdout, silent = False):
  ...
  if 'dram-datamovement-queue.num-cacheline-requests' in results and sum(results['dram-datamovement-queue.num-requests']) > 0:
    ...
    if 'dram-datamovement-queue.num-cacheline-requests-capped-by-window-size' in results and sum(results['dram-datamovement-queue.num-requests']) > 0:
      results['dram.remotequeuemodel_datamovement_percent_capped_by_window_size'] = map(lambda (a,b): 100*float(a)/b if b else float('nan'), zip(results['dram-datamovement-queue.num-page-requests-capped-by-window-size'], results['dram-datamovement-queue.num-page-requests']))
      results['dram.remotequeuemodel_datamovement_percent_queue_full'] = map(lambda (a,b): 100*float(a)/b if b else float('nan'), zip(results['dram-datamovement-queue.num-page-requests-queue-full'], results['dram-datamovement-queue.num-page-requests']))
      results['dram.remotequeuemodel_datamovement2_percent_capped_by_window_size'] = map(lambda (a,b): 100*float(a)/b if b else float('nan'), zip(results['dram-datamovement-queue.num-cacheline-requests-capped-by-window-size'], results['dram-datamovement-queue.num-cacheline-requests']))
      results['dram.remotequeuemodel_datamovement2_percent_queue_full'] = map(lambda (a,b): 100*float(a)/b if b else float('nan'), zip(results['dram-datamovement-queue.num-cacheline-requests-queue-full'], results['dram-datamovement-queue.num-cacheline-requests']))
      results['dram.remotequeuemodel_datamovement_queue_full_avg_injected_delay'] = map(lambda (a,b): float(a)/b if b else 0.0, zip(results['dram-datamovement-queue.page-requests-queue-full-injected-delay'], results['dram-datamovement-queue.num-page-requests-queue-full']))
      results['dram.remotequeuemodel_datamovement2_queue_full_avg_injected_delay'] = map(lambda (a,b): float(a)/b if b else 0.0, zip(results['dram-datamovement-queue.cacheline-requests-queue-full-injected-delay'], results['dram-datamovement-queue.num-cacheline-requests-queue-full']))
      ...
      results['dram.remotequeuemodel_datamovement_percent_capped_by_window_size'] = map(lambda (a,b): 100*float(a)/b if b else float('nan'), zip(results['dram-datamovement-queue.num-requests-capped-by-window-size'], results['dram-datamovement-queue.num-requests']))
      results['dram.remotequeuemodel_datamovement_percent_queue_full'] = map(lambda (a,b): 100*float(a)/b if b else float('nan'), zip(results['dram-datamovement-queue.num-requests-queue-full'], results['dram-datamovement-queue.num-requests']))
      results['dram.remotequeuemodel_datamovement_percent_capped_by_custom_cap'] = map(lambda (a,b): 100*float(a)/b if b else float('nan'), zip(results['dram-datamovement-queue.num-requests-capped-by-custom-cap'], results['dram-datamovement-queue.num-requests']))

    if 'dram-datamovement-queue.total-cacheline-queue-utilization-during-page-requests-numerator' in results:
      results['dram.avg_cacheline_queue_utilization_during_page_requests'] = map(lambda (a,b,c): float(a)/b /c if b and c else float('nan'), zip(results['dram-datamovement-queue.total-cacheline-queue-utilization-during-page-requests-numerator'], results['dram-datamovement-queue.total-cacheline-queue-utilization-during-page-requests-denominator'], results['dram-datamovement-queue.num-page-requests']))
  ...
  template += [
    ...
    ('  num extra traffic pages', 'dram.extra-pages', str),
    ('  num extra traffic cachelines', 'dram.extra-cachelines', str),
  ...
  template.extend([
      ...
      ('  PQ idea avg cacheline % util during page requests', 'dram.avg_cacheline_queue_utilization_during_page_requests', format_float(4)),
      ('  PQ idea avg page % util during cacheline requests', 'dram.avg_page_queue_utilization_during_cacheline_requests', format_float(4)),
      ('  PQ idea avg cacheline % util during page no-effect requests', 'dram.avg_cacheline_queue_utilization_during_page_no_effect', format_float(4)),
      ('  PQ idea avg page % util during cacheline no-effect requests', 'dram.avg_page_queue_utilization_during_cacheline_no_effect', format_float(4)),
      ...
      ('    datamovement avg queue full wait time', 'dram.remotequeuemodel_datamovement_queue_full_avg_injected_delay', format_ns(2)),
      ...
      ('    datamovement2 avg queue full wait time', 'dram.remotequeuemodel_datamovement2_queue_full_avg_injected_delay', format_ns(2)),
