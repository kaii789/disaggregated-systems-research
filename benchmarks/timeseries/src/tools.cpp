...
{
  std::fstream timeSeriesFile(infilename, std::ios_base::in);
	double tempval;
	timeSeriesLength = 0;
...
int saveProfileToFile(std::string outfilename, profile  * prof,  int timeSeriesLength, int windowSize)
{
  ...
  {
    preprofileOutFile << std::setprecision(std::numeric_limits<DTYPE>::digits10 + 2) << sqrt(abs(prof[i].value)) << " " << std::setprecision(std::numeric_limits<int>::digits10 + 1) << prof[i].idx << std::endl;
  }
