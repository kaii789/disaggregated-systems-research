...
def run_gran(config_name, compression_name, benchmark_name, x_axis, x_axis_label, x_axis_config_param, program_command, result_name, cwd=None
    ...
    # Multithread
    res1 = []
    t1 = threading.Thread(target=thread_experiment, args=(x_axis_label, x_axis, "{}-{}".format(compression_name, benchmark_name), os.path.abspath(config_name), x_axis_config_param, program_command, res1, cwd))
...
if __name__ == "__main__":
    ...
    compression_name = "bdi"
    for benchmark_name, program_command, cwd in program_info:
        t = threading.Thread(target=run_gran, args=("./bdi", compression_name, benchmark_name, bdi_gran, x_axis_label, x_axis_config_param, program_command, result_name, cwd))
        t.start()
    ...
    for benchmark_name, program_command, cwd in program_info:
        t = threading.Thread(target=run_gran, args=("./lz4", compression_name, benchmark_name, lz4_gran, x_axis_label, x_axis_config_param, program_command, result_name, cwd))
        t.start()
