...
def run_from_cmdline(
    ...
    while os.path.isfile(out_file_path):
        ...
        #            config_param_values.append(float(x_enum))
        #            raise ValueError(
        ...
        # cgiannoula
        config_param_values = range(1, 33, 2)
        first_file = False
        file_num += 1
        out_file_path = os.path.join(
            output_directory_path, "{}_sim.out".format(file_num)
...
def save_graph(
    ...
    title_str = "{}".format(config_param_name)
    plt.title(title_str)
    ...
    x_axis_labels = ["local-only", "local-cache", "pq=0.15", "pq=0.5", "ideal2x-lat0", "ideal-pq=0.15", "ideal-pq=bdi", "ideal2x-lat30", "ideal-pq=0.15", "ideal-pq=bdi", "bdi", "bdi-pq=0.15", "bdi-pq=bdi", "lz4", "lz4-pq=0.15", "lz4-pq=bdi"]
    plt.xticks(config_param_values, x_axis_labels, rotation=45)
