...
def run_from_experiment(output_directory_path, config_param_category, config_param_name, config_param_values):
        ...
        with open(out_file_path, "r") as out_file:
            ...
            if first_file:
                ...
                for line_no, line in enumerate(out_file_lines):
                    for index, stat_setting in enumerate(stat_settings):
                        if line.strip().startswith(stat_setting.line_beginning):
                            ...
                            y_values[index].append(stat_setting.format_func(line.split()[-1]))
                if not out_file_lines[ipc_line_no].strip().startswith("IPC") or None in y_value_line_nos:
                ...
                for index in range(len(y_values)):
                    ...
                    # The last entry of the line
                    y_values[index].append(stat_settings[index].format_func(line.split()[-1]))
...
def run_from_cmdline(output_directory_path, config_param_category, config_param_name):
    ipc_line_no = 3  # Indexing start from 0, not 1
            ...
            if first_file:
                ...
                for line_no, line in enumerate(out_file_lines):
                    for index, stat_setting in enumerate(stat_settings):
                        if line.strip().startswith(stat_setting.line_beginning):
                            y_value_line_nos[index] = line_no
                            y_values[index].append(stat_setting.format_func(
                                line.split()[-1]))  # The last entry of the line
                if not out_file_lines[ipc_line_no].strip().startswith("IPC") or None in y_value_line_nos:
                    print("Error: didn't find desired line in .out file")
                    sys.exit(-1)
            else:
                ...
                for index in range(len(y_values)):
                    line = out_file_lines[y_value_line_nos[index]]
                    y_values[index].append(stat_settings[index].format_func(
                        line.split()[-1]))  # The last entry of the line
...
def save_graph(output_directory_path, config_param_name, config_param_values, y_values, stat_settings):
    ...
    if len(line_style_list) < len(y_values):
        line_style_list.extend(
            [".--r" for _ in range(len(y_values) - len(line_style_list))])  # Extend list
    ...
    for i, (y_value_list, line_style) in enumerate(zip(y_values, line_style_list)):
        plt.plot(config_param_values, y_value_list,
